...
reproducible framework for growth curve analysis that interoperates with
Bioconductor data structures, enabling downstream integrative analyses.

`growkar` uses a canonical input format with columns `sample`, `time`, and
`od`, while still supporting common wide plate-reader style inputs.
...
For Bioconductor-oriented workflows, the same data can be represented as a
`SummarizedExperiment` while preserving the tidy API as the main user-facing
entry point.
