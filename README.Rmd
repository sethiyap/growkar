...
Graphing is optional: `ggplot2` and `RColorBrewer` are declared in `Suggests`,
so the data-structure and analysis layers install without a graphics stack. The
`plot_*()` functions check for them at call time and return standard `ggplot`
objects that can be customized into publication-ready figures.
